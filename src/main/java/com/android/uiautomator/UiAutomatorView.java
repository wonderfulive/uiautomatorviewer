...
public class UiAutomatorView extends Composite {
    ...
    public UiAutomatorView(Composite parent, int style, ScreenshotAction screenshotAction) {
        super(parent, SWT.NONE);
        setLayout(new FillLayout());
        mScreenshotAction = screenshotAction;
...
//        IDevice device = pickDevice();
        IDevice device = UiAutomatorHelper.getDevice();
        UiAutomatorHelper.exeClick(device, x, y);
        mScreenshotAction.run();
    }

    private void recordOperate(BasicTreeNode node) {
        //TODO record operate
    }
    ...
    private IDevice pickDevice() {
        ...
        if (devices.size() == 0) {
            ...
            ScreenshotAction.DevicePickerDialog dlg = new ScreenshotAction.DevicePickerDialog(getShell(), devices);
